...
tinymce.PluginManager.add("eqneditor", function(editor, url) {
  ...
  function showDialog() {
      ...
      items: [
        {
          name: "toolbar",
          type: "container",
          html: '<div style="padding:10px;"><div id="CCtoolbar' +
            CCinstance +
            '"></div>' +
            '<p style="margin-top:5px"><label for="CClatex' +
            CCinstance +
            '">Equation (LaTeX):</p>' +
            '<textarea id="CClatex' +
            CCinstance +
            '" rows="5" style="border:1px solid #8fb6bd; width:570px; font-size:16px; padding:5px; background-color:#ffc"></textarea>' +
            '<p style="margin-top:5px"><label for="CClatex' +
            CCinstance +
            '">Preview:</p>' +
            '<img id="CCequation' +
            ...
            '" src="' +
            http +
            'www.codecogs.com/images/spacer.gif" /></div>'
        }
      ],
      buttons: [
        /*{
        ...
        { type: "spacer", flex: 1 },
        {
          ...
          onclick: function() {
            editor.execCommand(
              "mceInsertContent",
              false,
              EqEditor.getTextArea().exportEquation("html")
            );
    ...
    EqEditor.add(
      new EqTextArea("CCequation" + CCinstance, "CClatex" + CCinstance),
      false
    );

    var imgElm = editor.selection.getNode();
    if (imgElm.nodeName == "IMG") {
      var sName = editor.dom
        .getAttrib(imgElm, "src")
        .match(/(gif|svg)\.latex\?(.*)/);
