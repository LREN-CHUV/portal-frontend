...
BB.App = function(el) {
  ...
  $(el).append(
    $(
      '<img src="svg/menus.png" title="Toggle Menus" style="position: absolute; left: 10px; top: 100%; margin-top: -34px; z-index: 2;" />'
    ).click(function() {
  ...
  $(rend.domElement)
    .bind("mouseup touchend", function(e) {
      var pageX = e.originalEvent.changedTouches
        ? e.originalEvent.changedTouches[0].pageX
        : e.pageX;
      var pageY = e.originalEvent.changedTouches
      ...
      down = false;
      if (
        Math.abs(pageX - downPos.x) <= 4 && Math.abs(pageY - downPos.y) <= 4
      ) {
        if (e.button == 2) {
...
BB.App.prototype.addMapper = function(mapper, cb) {
  ...
  var xhr = mapper.init(function() {
    ...
    var hide = $(
      '<input class="hide" type="button" value="Hide" title="Hide" />'
    );
    div.append(hide);
    var hideOthers = $(
      '<input class="hideOthers" type="button" value="Hide Others" title="Hide Others" />'
    );
    div.append(hideOthers);
    var transform = $(
      '<input class="transform" type="button" value="Transform" title="Transform (W/E/R to translate/rotate/scale, 2x to switch local/global)" />'
    );
    div.append(transform);
    var remove = $(
      '<input class="remove" type="button" value="Remove" title="Remove" />'
    );
    div.append(remove);
    ...
    remove.click(function() {
      ...
      if (controls !== undefined) {
        ...
        that.transformControls.splice(
          that.transformControls.indexOf(controls),
          1
        );
...
BB.App.prototype.saveSession = function(cb) {
  ...
  for (var i = 0; i < mappers.length; i++) {
    ...
    str +=
      '["' +
      mappers[i].getClassName() +
      '",' +
      JSON.stringify(mappers[i].serialize()) +
      "]";
  }
  str += "],";
  ...
  str +=
    JSON.stringify({
      cam: {
        ...
        rotation: { x: rot.x, y: rot.y, z: rot.z, order: rot.order || "XYZ" }
      },
      target: controls.target,
      object: { position: controls.object.position, up: controls.object.up }
    }) + "]";
...
BB.App.prototype.loadSession = function(id) {
  ...
  var that = this;
  DataFormats.getPSON("./load_session?id=" + encodeURIComponent(id), function(
    data
  ) {
    var mappers = data[0];
    var view = data[1];
