...
BB.GraphMapper.prototype.map = function(obj, el, cb) {
  ...
  for (var i = 0; i < positions.length; i++) {
    ...
    var plane = textTexture.createPlane(
      ret
  ...
  var highlightObject = new THREE.Mesh(
  ...
  $(el).append(
    $('<input type="button" value="Stop" />').click(function() {
      if ($(this).val() == "Stop") {
        $(this).val("Continue");
  ...
  $(el)
    ...
    .append(
      $('<input type="button" value="More" />').click(function() {
        if (that.activeObject.userData === undefined) {
          ...
          if (dataType == "Genetic") {
            url =
              ...
              encodeURIComponent(label.match("(rs[0-9]*)")[1]);
            ...
            url =
              "http://www.uniprot.org/uniprot/?query=" +
              encodeURIComponent(label.match("Protein: (.*)")[1]);
    ...
    .append(
      $('<input type="button" value="Only" />').click(function() {
    ...
    .append(
      $('<input type="button" value="Outgoing" />').click(function() {
        var active = that.active;
        var idx = that.activeObject.userData.idx;
        for (var i = 0; i < sources.length; i++) {
    ...
    .append(
      $('<input type="button" value="Incoming" />').click(function() {
        var active = that.active;
        var idx = that.activeObject.userData.idx;
        for (var i = 0; i < sources.length; i++) {
          var src = idToOfs[sources[i]];
    ...
    .append(
      $('<input type="button" value="Expand" />').click(function() {
        // var idx = that.activeObject.userData.idx;
    ...
    .append(
      $('<input type="button" value="No islands" />').click(function() {
...
BB.GraphMapper.prototype.getNode = function(e, cb) {
  ...
  for (var i = 0; i < isect.length; i++) {
    ...
    if (
      isect[i].object.visible && isect[i].object.userData.label !== undefined
    ) {
      cb(isect[i].object);
    }
  }
...
BB.TextTexture = function(width, height, font, fontHeight) {
  ...
  this.materials[0] = new THREE.ShaderMaterial({
    defines: { COMPONENT: "0" },
    uniforms: { map: { type: "t", value: this.texture } },
    vertexShader: BB.TextTexture.vertexShader,
    fragmentShader: BB.TextTexture.fragmentShader,
    depthTest: false,
    depthWrite: false,
    transparent: true
  });
  this.materials[1] = new THREE.ShaderMaterial({
    defines: { COMPONENT: "1" },
    uniforms: { map: { type: "t", value: this.texture } },
    vertexShader: BB.TextTexture.vertexShader,
    fragmentShader: BB.TextTexture.fragmentShader,
  ...
  this.materials[2] = new THREE.ShaderMaterial({
    defines: { COMPONENT: "2" },
    uniforms: { map: { type: "t", value: this.texture } },
    vertexShader: BB.TextTexture.vertexShader,
    fragmentShader: BB.TextTexture.fragmentShader,
...
BB.TextTexture.prototype.createPlane = function(ret) {
  ...
  plane.faceVertexUvs[0][0] = [
    new THREE.Vector2(ret[0], ret[1]),
    new THREE.Vector2(ret[0], ret[3]),
    new THREE.Vector2(ret[2], ret[1])
  ];
  plane.faceVertexUvs[0][1] = [
    new THREE.Vector2(ret[0], ret[3]),
    new THREE.Vector2(ret[2], ret[3]),
    new THREE.Vector2(ret[2], ret[1])
  ];
  return plane;
};
