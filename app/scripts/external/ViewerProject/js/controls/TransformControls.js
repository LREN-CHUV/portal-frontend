...
THREE.TransformControls = function(camera, domElement, doc) {
  ...
  // gizmo geometry
  {
    ...
    mesh = new THREE.Mesh(
      new THREE.TorusGeometry(1.30, 0.15, 4, 12),
      HandleMaterial(yellow, 0.25)
    );
    mesh.name = "RE";
    pickerAxes["rotate"].add(mesh);
  ...
  this.attach = function(object) {
    ...
    this.domElement.eventProxy.addBefore("mousedown", onMouseDown, false);
    this.domElement.eventProxy.addBefore("mousemove", onMouseHover, false);
    this.document.addEventListener("keydown", onKeyDown, false);
  };

  this.detach = function(object) {
    this.previousObject = this.object || this.previousObject;
    this.object = undefined;
    this.hovered = false;
  ...
  this.update = function() {
    ...
    camRotation.setFromRotationMatrix(
      tempMatrix.extractRotation(this.camera.matrixWorld)
    );
  ...
  function onKeyDown(event) {
    ...
    if (event.keyCode == 87) {
      ...
      if (scope.mode == "translate")
        scope.space = scope.space == "world" ? "local" : "world";
    ...
    if (
      ...
      scope.space !== currentSpace ||
      scope.scale !== currentScale
    ) {
