...
THREE.TrackballControls = function(object, domElement) {
  ...
  this.zoomCamera = function() {
    if (_state === STATE.TOUCH_ZOOM) {
      ...
      if (factor !== 1.0 && factor > 0.0) {
        _eye.multiplyScalar(factor);
        //var pan = _eye.clone().normalize().multiplyScalar((_zoomEnd.y - _zoomStart.y) * _this.zoomSpeed * _this.screen.height);
  ...
  this.myReset = function() {
    lastPosition.copy(this.object.position);
    this.target.set(0, 0, 0);

    _eye.set(0, 0, 0);

    _rotateStart.set(0, 0, 0);
    _rotateEnd.set(0, 0, 0);

    _zoomStart.set(0, 0);
    _zoomEnd.set(0, 0);

    (_touchZoomDistanceStart = 0), (_touchZoomDistanceEnd = 0), _panStart.set(
      0,
      0
    );
    _panEnd.set(0, 0);
