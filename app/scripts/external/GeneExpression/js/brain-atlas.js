...
function select(event) {
  if (selected) {
    for (var i = 0; i < svgStructs.length; i++) {
      if (
        svgStructs[i].getAttribute("structure_id") ==
        selected.attr("structure_id")
      ) {
  ...
  $("#editbox_search_structure_id").val(
    structureLUT[$(event.currentTarget).attr("structure_id")]
  );
}
...
function out(event) {
  var current = $(event.currentTarget);
  if (
    !selected || current.attr("structure_id") != selected.attr("structure_id")
  ) {
