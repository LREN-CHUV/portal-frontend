...
function addBox(divId) {
  ...
  var containerWidth = parseFloat(
    d3.select(divId).selectAll("svg").style("width")
  );
      ...
      svgContainer
        ...
        .attr(
          ...
          "rotate(-90 " +
            (x - width / 2) +
            "," +
            (yInit - height - labelsAdjustment) +
            ")"
        )
        .attr("font-size", width * 0.6)
        .text(ageLabel);
    }
  }
...
function updateUI() {
  ...
  if (!(gene == "" || structure == "")) {
    ...
    var baseURLGeneExpr =
      "http://hbps4.chuv.ch:8080/services-gensign/gene_expression/query?";
    var baseURLCorrelAll =
      "http://hbps4.chuv.ch:8080/services-gensign/correlation_all/query?";
    var baseURLCorrelAge =
      "http://hbps4.chuv.ch:8080/services-gensign/correlation_timeframe/query?";
...
function resizeend() {
  if (new Date() - rtime < delta) {
    ...
    if (browserWidth != newWidth) {
      ...
      // Resize brain atals
      svgDoc
        .getElementById("brain_schematic_svg")
        .setAttribute("transform", "scale(" + scale + ")"); // DO NOT USE JQuery for that -> http://benfrain.com/selecting-svg-inside-tags-with-javascript/
