  ...
  .directive("criteriaChainedSelect", function() {
    return {
      ...
      controller: [
        ...
        "$log",
        "$rootScope",
        function($scope, filterFilter, $log, $rootScope) {
          $scope.selectedList = [];
          $scope.isInit = false;
         ...
         */
          $scope.init = function(criteria) {
            ...
            if (
              variable === undefined ||
              variable === null ||
              (!_.isArray(variable) && variable.code === undefined)
            ) {
              return;
            }
         ...
         */
          $scope.initSelectedVariablesByGroup = function(group) {
            if (group.groups.length == 0) {
              $scope.selectedVariablesByGroup = $scope.getVariablesOfGroup(
                $scope.selectedList,
                $scope.variables
              );
            }
            ...
            angular.forEach(groups, function(currentGroup, index) {
              if (currentGroup.isVariable === undefined) {
                if (selectedGroup === undefined) {
                  ...
                  filterArray = filterFilter(filterArray, {
         ...
         */
          $scope.getVariablesOfGroup = function(groups, variables) {
            var result = groups.map(function(group) {
              return _.filter($scope.variables, function(variable) {
                return variable.group.code == group.code;
              });
            });
         ...
         */
          $scope.setCriteria = function(v) {
            ...
            if ($scope.useGroupCriteria && last.isVariable === undefined) {
              $scope.criteria = _.map($scope.selectedVariablesByGroup, function(
                variable
              ) {
            ...
            function(newValue) {
              ...
              $scope.$watch("criteria", function(newValue, oldValue) {
                if (
                  !$scope.isInit &&
                  newValue !== undefined &&
                  (newValue.length === undefined || newValue.length > 0) &&
