      ...
      link: function(scope, element) {
        scope.loadCarrousel = function() {
          ...
          if ($panel_article_search_widget.length) {
            $panel_article_search_widget.width(
              $panel_article_search_widget.parent().width()
            );
            var panel_write_article_top_position = $(
              ".panel-form-write-article"
            ).offset().top;
            if (
              panel_write_article_top_position - $(window).scrollTop() <= 100
            ) {
              $panel_article_search_widget.css("top", 100);
            } else {
              $panel_article_search_widget.css(
                "top",
                panel_write_article_top_position - $(window).scrollTop()
              );
            }
...
angular.module("chuvApp.components.carrousel").directive("carrouselChart", [
  "ChartUtil",
  "Model",
  "backendUrl",
  "$compile",
