...
angular.module("chuvApp.mydata").controller("MyDataController", [
  "$scope",
  ...
  function(
    ...
    $scope.init = function() {
      var paramsArticle = {
        team: $scope.isMyDataScope ? 0 : 1,
        own: !$scope.isMyDataScope ? 0 : 1
      };
      ...
      $q
        .all([
          Article.query(paramsArticle).$promise,
          Model.query(paramsModel).$promise
        ])
        .then(function(data) {
          var articles = data[0];
          var models = data[1];
            ...
            $q
              .all(
                _.map(models, function(model) {
                  return $scope.getModel(model.slug);
                })
              )
              .then(function(modelsData) {
                ...
                if (articles.length > modelsData.length) {
                  angular.forEach(articles.slice(modelsData.length), function(
                    article,
                    index
                  ) {
                    $scope.rows.push({
