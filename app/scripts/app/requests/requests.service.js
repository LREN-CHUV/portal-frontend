...
angular.module("chuvApp.requests").factory("Request", [
  "$rootScope",
  function() {
    var datas = {
      facets: [
      ...
      results: [
        {
          ...
          author_obj: {
            id: 1,
            fullname: "Henry Markram",
            picture: "images/authors/Henry-Markram_001.png"
          }
        },
        {
          id: 2,
          ...
          author_obj: {
            id: 3,
            fullname: "Anastasia Ailamaki",
            picture: "images/authors/75530b64.jpg"
          }
        },
        {
          id: 3,
    ...
    return {
      search: function(label, filters) {
        ...
        if (
          (filters != null && Object.keys(filters).length > 0) || label != null
        ) {
          var requests = [];
          for (var index in result.results) {
            if (result.results.hasOwnProperty(index)) {
              ...
              if (
                label != null &&
                request.label.toLowerCase().search(label.toLowerCase()) == -1
              ) {
                validResult = false;
              }
              for (var key in filters) {
                if (
                  ...
                  (filters[key].indexOf(request[key]) == -1 &&
      ...
      get: function(id) {
        for (var idx in datas.results) {
          if (
            datas.results.hasOwnProperty(idx) && datas.results[idx].id == id
          ) {
      ...
      update: function(request) {
        ...
        for (var idx in datas.results) {
          if (
            datas.results.hasOwnProperty(idx) &&
            datas.results[idx].id == request.id
          ) {
