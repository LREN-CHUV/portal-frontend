...
angular.module("chuvApp.models").controller("DatasetController", [
  ...
  function(
      ...
      Config.then(config => config.mode === "local").then(isLocal => {
        ...
        $scope.allDatasets.forEach(d =>
          Model.mining({
            ...
            .then(r => {
              const data =
                (r && r.data && r.data.data && r.data.data.data) || []; // return a "placeholder" for the non catched dataset error
              const dataset = {
                data,
                name: d.code,
    ...
    const getBoxplot = data => {
      ...
      const hasGenderInVariables = $scope.query.groupings
        .map(d => d.code)
        .includes("gender");

      const filterByVariable = datasets => ({
        variables: [...$scope.query.variables, ...$scope.query.coVariables].map(
          variable => ({
            variable,
