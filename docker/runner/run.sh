...
if [ ! -z "$PORTAL_BACKEND_SERVER" ]; then
  OPTS="$OPTS -wait tcp://${PORTAL_BACKEND_SERVER} -timeout 100s -template /portal/conf/portal-backend-upstream.conf.tmpl:/etc/nginx/conf.d/portal-backend-upstream.conf"
